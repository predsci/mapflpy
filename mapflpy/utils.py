...
from pyvisual.core.tracer import Tracer, fetch_default_launch_points, Traces


def extract_ordered_seqs(directory, variable_id):
    dtype = np.dtype([('seq', 'i4'),  # int32
                      ('file', 'U256')])
    files = glob.glob(os.path.join(directory, f'{variable_id}*'))
    seqs_vr = np.array([
        (int(os.path.splitext(f.split(variable_id)[-1])[0]), f) for f in files
    ], dtype=dtype)
    return np.sort(seqs_vr, order='seq')
