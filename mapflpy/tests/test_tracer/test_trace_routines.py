...
def test_tracing_against_reference_traces(tracer_instance, mesh_fields_asarray, launch_points, default_lps_params, reference_traces):
    ...
    match lps_id:
        case 'fwd':
            ...
            traces = tracer_instance.trace(lps, buffer_size=default_lps_params['BUFFER'])
        case 'bwd':
            tracer_instance.set_tracing_direction('b')
            traces = tracer_instance.trace(lps, buffer_size=default_lps_params['BUFFER'])
        case 'both':
            tracer_instance.set_tracing_direction('f')
            fwd_traces = tracer_instance.trace(lps, buffer_size=default_lps_params['BUFFER'])
            tracer_instance.set_tracing_direction('b')
            bwd_traces = tracer_instance.trace(lps, buffer_size=default_lps_params['BUFFER'])
